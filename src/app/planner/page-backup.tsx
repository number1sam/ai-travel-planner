...
export default function PlannerPage() {
  ...
  const selectCitiesForDestination = (destination: string, duration: number, month: string): { cities: string[], reasoning: string } => {
    ...
    const cityRecommendations: Record<string, any> = {
      ...
      'tenerife': {
        short: { cities: ['Tenerife'], reasoning: 'Tenerife offers the perfect Canary Islands experience with stunning beaches, volcanic landscapes, and year-round sunshine.' },
        medium: { cities: ['Tenerife'], reasoning: 'Focus on Tenerife to fully explore this diverse island - from Mount Teide to beautiful beaches and charming towns.' },
        long: { cities: ['Tenerife'], reasoning: 'Immerse yourself completely in Tenerife\'s unique blend of volcanic wonders, coastal beauty, and Spanish island culture.' }
      },
      'canary islands': {
        short: { cities: ['Tenerife'], reasoning: 'Tenerife is the largest and most diverse of the Canary Islands, perfect for a focused island getaway.' },
        medium: { cities: ['Tenerife', 'Gran Canaria'], reasoning: 'Experience two distinct Canary Islands - Tenerife for volcanic landscapes and Gran Canaria for diverse terrains.' },
        long: { cities: ['Tenerife', 'Gran Canaria', 'Lanzarote'], reasoning: 'Explore the variety of the Canary Islands from Tenerife\'s peaks to Lanzarote\'s unique volcanic formations.' }
      }
    }
  ...
  const generateComprehensiveTripPlan = async () => {
    ...
    try {
      ...
      // Debug logging for destination
      console.log('🔍 DEBUG: Destination parsing results:')
      console.log('   Original destination:', requiredTripData.destination)
      console.log('   Cities list:', cities)
      console.log('   Destination info:', destinationInfo)

      // Send hotel search progress message
      const hotelSearchMessage: Message = {
        id: `hotel-search-${Date.now()}`,
        text: `🏨 **Searching for real hotels in ${cities.join(' and ')}...**\\n\\nI'm checking availability and prices from multiple booking platforms to find the best accommodations that match your budget and preferences.`,
        sender: 'ai',
        timestamp: new Date()
      }
      setMessages(prev => [...prev, hotelSearchMessage])
      ...
      const hotelResults = await Promise.all(cities.map(async (city, index) => {
        ...
        console.log(`🔍 REAL-TIME HOTEL SEARCH: Starting structured search for ${city}`)
        
        // Only search real hotels if user wants hotels, not Airbnbs/hostels/camping
        if (!wantsHotels) {
          console.log(`📌 User wants ${requiredTripData.accommodationType || 'other accommodation'}, not using hotel database`)
          return {
            success: false,
            hotels: [{
              name: `${city} ${requiredTripData.accommodationType === 'airbnb' ? 'Airbnb' : requiredTripData.accommodationType === 'hostel' ? 'Hostel' : 'Accommodation'}`,
              type: requiredTripData.accommodationType || 'hotel',
              location: `${city} City Center`,
              rating: 4.0,
              pricePerNight: Math.floor(budgetBreakdown.accommodation / requiredTripData.duration!),
              amenities: ['WiFi'],
              checkIn: cityStartDate.toISOString().split('T')[0],
              checkOut: cityEndDate.toISOString().split('T')[0],
              nights: Math.ceil((cityEndDate.getTime() - cityStartDate.getTime()) / (1000 * 60 * 60 * 24))
            }]
          }
        }
        
        // ✅ STRUCTURED DATA RETRIEVAL - NO LLM INVOLVEMENT
        try {
          // Import hotel data manager
          const { HotelDataManager } = await import('@/services/HotelDataManager')
          
          const nights = Math.ceil((cityEndDate.getTime() - cityStartDate.getTime()) / (1000 * 60 * 60 * 24))
          const maxNightlyRate = Math.floor(budgetBreakdown.accommodation / requiredTripData.duration!)
          
          // Create REAL-TIME query for OTA APIs
          const hotelQuery = {
            location: city,
            checkIn: cityStartDate.toISOString().split('T')[0],
            checkOut: cityEndDate.toISOString().split('T')[0],
            guests: requiredTripData.travelers!,
            rooms: Math.ceil(requiredTripData.travelers! / 2),
            currency: 'GBP',
            budget: {
              min: Math.floor(maxNightlyRate * 0.5),
              max: maxNightlyRate
            },
            filters: {
              minStarRating: requiredTripData.accommodationType === 'luxury' ? 4 : 3,
              minReviewScore: 7.0,
              amenities: ['WiFi']
            }
          }
          
          // Define selection criteria (outside of LLM)
          const selectionCriteria = {
            prioritizePrice: requiredTripData.budget! < 1500,
            prioritizeReviews: true,
            prioritizeLocation: true,
            prioritizeAmenities: requiredTripData.accommodationType === 'luxury',
            mustHaveAmenities: requiredTripData.accommodationType === 'luxury' ? ['WiFi', 'Restaurant'] : ['WiFi'],
            avoidProperties: [],
            preferredChains: requiredTripData.accommodationType === 'luxury' ? ['Marriott', 'Hilton', 'Hyatt'] : []
          }
          
          // 🔒 SELECT AND LOCK hotel data (prevents LLM tampering)
          const lockedSelection = await HotelDataManager.selectAndLockHotel(
            'user123', // In production, use actual user ID
            hotelQuery,
            selectionCriteria,
            'trip_' + Date.now()
          )
          
          const selectedHotel = lockedSelection.selectedHotel
          
          console.log(`✅ LOCKED HOTEL SELECTED: ${selectedHotel.name} - £${selectedHotel.pricePerNight}/night`)
          
          return {
            success: true,
            selectionId: lockedSelection.id, // Store for later validation
            hotels: [{
              name: selectedHotel.name,
              type: requiredTripData.accommodationType || 'hotel',
              location: selectedHotel.address,
              rating: selectedHotel.reviewScore / 2, // Convert 10-point to 5-point
              pricePerNight: selectedHotel.pricePerNight,
              amenities: selectedHotel.amenities,
              checkIn: hotelQuery.checkIn,
              checkOut: hotelQuery.checkOut,
              nights: nights,
              bookingLink: selectedHotel.deepBookingLink, // REAL booking link
              reviews: selectedHotel.reviewCount,
              description: `${selectedHotel.starRating}-star hotel with ${selectedHotel.reviewScore}/10 rating`,
              stars: selectedHotel.starRating,
              coordinates: selectedHotel.coordinates,
              cancellationPolicy: selectedHotel.cancellationPolicy,
              selectionReason: lockedSelection.selectionReason
            }],
            alternatives: lockedSelection.alternatives.slice(0, 2).map(alt => ({
              name: alt.name,
              rating: alt.reviewScore / 2,
              pricePerNight: alt.pricePerNight,
              location: alt.address,
              link: alt.deepBookingLink
            }))
          }
          
        } catch (realTimeError) {
          console.error(`❌ Real-time hotel search failed for ${city}:`, realTimeError)
        }
        
        // Define variables for fallback and additional processing
        const nights = Math.ceil((cityEndDate.getTime() - cityStartDate.getTime()) / (1000 * 60 * 60 * 24))
        const maxNightlyRate = Math.floor(budgetBreakdown.accommodation / requiredTripData.duration!)
        
        // Create search profile for fallback hotel search
        const searchProfile: SearchProfile = {
            destinationCity: city,
            country: destinationInfo?.country || 'Unknown',
            checkIn: cityStartDate.toISOString().split('T')[0],
            checkOut: cityEndDate.toISOString().split('T')[0],
            guests: requiredTripData.travelers!,
            nights: nights,
            totalBudget: budgetBreakdown.accommodation,
      ...
      // Send hotel found message with details
      const foundHotels = hotelResults.filter(h => h.success).map((h, i) => ({
        city: cities[i],
        hotel: h.hotels[0]
      }))
      
      if (foundHotels.length > 0) {
        let hotelFoundMessage = `✅ **Found excellent hotels for your stay:**\\n`
        foundHotels.forEach(({ city, hotel }) => {
          hotelFoundMessage += `\\n📍 **${hotel.name}** in ${city}\\n`
          hotelFoundMessage += `   • ${hotel.stars || 3}-star hotel, rated ${hotel.rating}/5\\n`
          hotelFoundMessage += `   • £${hotel.pricePerNight}/night\\n`
          hotelFoundMessage += `   • ${hotel.location}\\n`
        })
        
        const hotelFoundMsg: Message = {
          id: `hotel-found-${Date.now()}`,
          text: hotelFoundMessage,
          sender: 'ai',
          timestamp: new Date()
        }
        setMessages(prev => [...prev, hotelFoundMsg])
      }
      ...
      // Extract hotel information for the success message
      const selectedHotels = hotelResults.filter(h => h.success).map(h => h.hotels[0])
      let hotelSummary = ''
      if (selectedHotels.length > 0) {
        const primaryHotel = selectedHotels[0]
        hotelSummary = `\\n\\n🏨 **Your Accommodation:**\\n`
        
        selectedHotels.forEach((hotel, index) => {
          if (hotel) {
            const cityName = cities[index] || 'your destination'
            hotelSummary += `\\n**${hotel.name}** in ${cityName}\\n`
            hotelSummary += `• ${hotel.stars || 3}-star hotel, rated ${hotel.rating}/5 ${hotel.reviews ? `(${hotel.reviews} reviews)` : ''}\\n`
            hotelSummary += `• £${hotel.pricePerNight}/night • ${hotel.location}\\n`
            if (hotel.amenities && hotel.amenities.length > 0) {
              hotelSummary += `• ${hotel.amenities.slice(0, 3).join(', ')}\\n`
            }
            if (hotel.description) {
              hotelSummary += `• "${hotel.description}"\\n`
            }
            if (hotel.bookingLink) {
              hotelSummary += `• ${hotel.cancellationPolicy || 'Standard cancellation policy'}\\n`
            }
          }
        })
        
        hotelSummary += `\\n✨ Selected for excellent reviews, location, and value within your budget!`
      } else {
        hotelSummary = `\\n\\n🏨 **Hotels:** Comfortable accommodations selected for all ${requiredTripData.duration} nights`
      }
